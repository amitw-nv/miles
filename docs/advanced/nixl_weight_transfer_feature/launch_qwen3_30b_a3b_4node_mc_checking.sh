#!/bin/bash
# =============================================================================
# launch_qwen3_30b_a3b_4node_mc_checking.sh
...
#     bash examples/infra_features/p2p_weight_transfer/Qwen3-30B-A3B.sh p2p 1 $HEAD_NODE_IP  # worker
# That one wrapper script already does prepare-if-missing + run.py run, so this
# launcher does not call run.py directly.
#
...
# USAGE
# -----
#   bash launch_qwen3_30b_a3b_4node_mc_checking.sh [MODE]
#     MODE : p2p (default) | broadcast   (mainline has no "nixl" mode)
#
# MONITORING
# ----------
#   tail -f /lustre/fsw/portfolios/network/users/amitw/miles/logs/miles-qwen3-30b-a3b-mc-checking-<JOBID>.out
#   squeue -u amitw
#   scancel <JOBID>
# =============================================================================
...
C_NAME=amitw-miles-mc-checking-4node-100926
LOG_DIR=$LUSTRE/logs
...
echo ""
echo "=========================================="
echo "  Miles cluster launch — Qwen3-30B-A3B 4-node (mainline MC-checking)"
echo "  Mode      : $MODE"
...
JOB_SCRIPT=$LOG_DIR/job-qwen3-30b-a3b-mc-checking.slurm
cat > "$JOB_SCRIPT" <<SLURM
#!/bin/bash
#SBATCH --job-name=miles-qwen3-30b-a3b-mc-checking
...
bash examples/infra_features/p2p_weight_transfer/Qwen3-30B-A3B.sh $MODE "\$NODE_RANK" "\$HEAD_NODE_IP"
'
SLURM

JOB_ID=$(sbatch --parsable "$JOB_SCRIPT")
echo "Submitted job $JOB_ID"
echo "  tail -f $LOG_DIR/miles-qwen3-30b-a3b-mc-checking-${JOB_ID}.out"
echo "  squeue -u amitw"
echo "  scancel $JOB_ID"
